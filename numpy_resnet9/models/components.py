...
class bn_layer:
    def __init__(self, num_features, momentum = 0.9):
        self.gamma = np.random.uniform(low=0, high=1, size=num_features).astype(np.float32)
        self.bias = np.zeros([num_features], dtype=np.float32)
        self.moving_avg = np.zeros([num_features])
...
class FC:   # without sigmoid
    ...
    def init_param(self):   # Xavier初始化
        self.kernel = np.random.uniform(
            low = -np.sqrt(6.0 / (self.out_features + self.in_features)),
            high = np.sqrt(6.0 / (self.in_features + self.out_features)),
    ...
    def backward(self, out_diff_tensor):    # out_diff_tensor：损失函数对y的导数
        ...
        kernel_diff = np.dot(self.in_tensor.T, out_diff_tensor).T
        # bias_diff = np.sum(out_diff_tensor, axis=0).reshape(self.bias.shape)
        bias_diff = np.mean(out_diff_tensor, axis=0).reshape(self.bias.shape)
