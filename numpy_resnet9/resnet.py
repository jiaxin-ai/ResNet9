...
# pynvml.nvmlInit()
# # 获取第一个 GPU 设备句柄
# handle = pynvml.nvmlDeviceGetHandleByIndex(0)  # 0 表示第一个 GPU，若有多个 GPU，可以指定对应的索引
...
# pid = os.getpid()
# # 获取当前进程的显存使用情况
# handle = pynvml.nvmlDeviceGetHandleByIndex(0)  # 假设使用第 0 个 GPU
    ...
    def train(self,process):
        ...
        for epoch in range(self.start_epoch, 1+self.num_epochs):
            ...
            for i, data in enumerate(self.train_dataloader):
                ...
                iter_info_str = f'Epoch: {epoch}, Step:{i}, Time: {time.time() - iter_start_time}, Loss: {loss}, Batch Accuracy: {batch_correct / batch_total}, Running Epoch Accuracy: {epoch_correct / epoch_total}, CPU usage: {cpu_usage}%, GPU memory usage: {gpu_memory_usage} MB, GPU Util: {gpu_util}'
