...
def cross_entropy_loss_with_gradient(logits, labels):
    ...
    softmax_out = logits_exp / np.sum(logits_exp, axis=1, keepdims=True)

    # 2. 计算交叉熵损失
    N = logits.shape[0]
    p_label = softmax_out[np.arange(N), labels.squeeze(-1)]

    # 避免 log(0) 的情况，加一个小的 epsilon
    epsilon = 1e-12
    loss = -np.log(p_label + epsilon)  # 负对数损失
    # loss_mean = np.sum(loss) / N # 平均损失
    loss_mean = np.mean(loss)  # 平均损失

    # 3. 计算梯度
    # 初始化梯度为 softmax 输出
    gradient = softmax_out.copy()
    # 将真实标签对应的位置减去 1
    gradient[np.arange(N), labels.squeeze(-1)] -= 1
    # 计算平均梯度
    gradient = gradient / N
...
if __name__ == '__main__':
    # 示例 logits
    x = np.array([[1.2, 3.4, 0.3, 5.3, 0.4],
                  [1.6, 0.4, 2.3, 4.3, 3.4]])
