...
class DataLoader:
    def __init__(self, data, batch_size, shuffle=True, drop_last=False):
        ...
        # self.indices = cp.arange(self.num_samples)  # 样本索引
    ...
    def __next__(self):
        ...
        for i in range(len(batch_data)):
            batch_data[i] = cp.stack(batch_data[i], axis=0)
    ...
    def _extract_batch(self, batch_indices):
        ...
        batch = [cp.zeros((len(batch_indices), *cp.shape(element))) for element in first_sample]
